...
function App() {
  ...
  const { showGLSL, setShowGLSL, showTS, setShowTS } = useGlobalState() as GlobalState
  ...
  return (
    ...
    >
      ...
      {!showCoodeMenu ? (
        <>
          <Button
            className={`circle show-code mt-5 ml-20 absolute z-8 ${showGLSL ? '' : 'inactive'}`}
            onClick={() => setShowGLSL(!showGLSL)}
          >
            GLSL
          </Button>
          <Button
            className={`circle show-code mt-5 ml-34 absolute z-8 ${showTS ? '' : 'inactive'}`}
